...
class Database:
    ...
    async def get_daily_downloads(self, user_id):
        user_data = await self.users.find_one({"id": user_id})
        if user_data:
            last_download_date = user_data.get("last_download_date", None)
            if last_download_date and last_download_date.date() != datetime.datetime.now(pytz.timezone("Asia/Kolkata")).date():
                await self.users.update_one({"id": user_id}, {"$set": {"daily_downloads": 0}})
                return 0
            return user_data.get("daily_downloads", 0)
        return 0
    ...
    async def increment_daily_downloads(self, user_id):
        await self.users.update_one(
            {"id": user_id},
            {
                ...
                "$set": {"last_download_date": datetime.datetime.now(pytz.timezone("Asia/Kolkata"))}
            },
            upsert=True
        )
